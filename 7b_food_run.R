...
scenarios_areas <- scenarios_areas[!lcm %chin% c("b.t", "b.w", "c.w", "c.w_pinewood", "c.l", "f.w", "a.h_palud", "i.g_palud", "r.k", "a.h_energy", "i.g_energy",
                                                 "wind_c.w", "wind_a.h_energy", "wind_a.h_palud", "wind_i.g_energy", "wind_i.g_palud", "solar")
...
food_results_basic <- scenarios_areas[, .N, keyby = c("scenario", "year")
][, N := NULL] %>% 
  as_tibble() %>%
  mutate(i = 1:nrow(.)) %>%
  group_by(i) %>%
...
# Save
save(food_results_basic, file = here("outputs", "food_results_basic_all.RData"))
