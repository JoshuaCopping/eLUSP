...
get_areas_loops <- function(country_abbreviation){
  ...
  # Scenarios areas blank table - populates in loops
  ...
  # Scenarios change areas blank table - populates in loops
  ...
  for(j in 1:nrow(country_df)){
    ...
    country_abbr <- country_df$name[j]
    ...
    # Load updated LCM per country in loop
    r_lcm_stack <- stack(here("rasters", "max wind", glue("r_renewable_lcm_{country_abbr}_i_np_max.tif")))
    
    .scenario <- str_sub(names(r_lcm_stack[[1]]), 21) %>% 
      str_sub(end = -3)
    ...
    # Progress update
    print(paste("Finished loop for", country_abbr, "-", Sys.time()))
  ...
  # SAVE OUTPUTS ----
  save(scenarios_areas, scenarios_change_areas, file = here("outputs", "scenario areas", "scenarios_areas_max.RData"))
...
get_areas_loops(c("eng", "nir", "sct", "wal"))
