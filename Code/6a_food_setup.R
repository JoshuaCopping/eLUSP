...
agroforestry_footprint <- read_csv(here("data", "agroforestry_footprint.csv"), skip = 1) %>% select(-notes)
...
## PARAMS FOR TESTING FUNCTION
# areas <- scenarios_areas %>%
#   filter(year == 2050 & scenario %in% c("i_np_max")) %>%
...
# .feed_multiplier <- 1
# .name <- NA
# x <- areas#scenarios_areas %>% filter(scenario == 1 & year == 2020)
...
food_production_fun <- function(x, params, .waste_multiplier = 1, .feed_multiplier = 1, .name = NA, waste = FALSE){
  ...
  if(length(params$.min_legumes) <= 1){
    ...
    .min_legumes <- params$.min_legumes %>% 
      filter(year == unique(x$year) & scenario == unique(x$scenario)) %$%
      min_legumes
  }
  ...
  ## Fodder crops ---------------
  # Calculate arable areas from land use data
  arable_areas <- x %>%
...
# SAVE ---------------------
save(food_production_fun, file = here("data", "food_fun_2022.RData"))
