...
## UPDATE SCENARIOS AREAS SCENARIOS NAMES
...
## Intertidal
load(here("data", "ghg_pars_intertidal.RData"))

## Peat
load(here("data", "ghg_pars_peat.RData"))

## Bioenergy
load(here("data", "ghg_pars_bioenergy.RData"))
...
# Energy ambition for sub-setting food results - not needed in running or model script
# energy_ambition <- "low" # use high or low
...
do_ghg_fun <- function(scenarios_areas, scenarios_change_areas,
  ...
  luc_areas <- scenarios_change_areas[!lcm_to %chin% c("s.m", "b.g", "f.s", "a.h_palud", "i.g_palud")
  ...
  ][, ':='(area = ifelse(lcm_to %chin% c("wind_a.h_energy", "wind_i.g_energy"), area*(1-wind_footprint), area),
           lcm_from = ifelse(lcm_from %chin% c("wind_a.h_energy", "wind_i.g_energy"), substring(lcm_from, first = 6), lcm_from),
           lcm_to = ifelse(lcm_to %chin% c("wind_a.h_energy", "wind_i.g_energy"), substring(lcm_to, first = 6), lcm_to))
  ][!(str_detect(lcm_to, "wind|solar"))
    ...
    , keyby = .(scenario, lusp_scenario, year, lcm_from, lcm_to, country)
    , .(area = sum(area))] %>% 
    as_tibble() %>% 
    ...
    select(-year) %>% 
    rename(year = year_) %>%
    arrange(scenario, lusp_scenario, year, country)  
  
  # Calculate land-use change areas - RENEWABLES
  luc_renewable_areas <- scenarios_change_areas[str_detect(lcm_to, "wind|solar")
  # Rename
  ][, ':='(lcm_to = ifelse(str_detect(lcm_to, "wind"), "wind", "solar"),
           lcm_from = ifelse(str_detect(lcm_from, "wind"), "wind", ifelse(str_detect(lcm_from, "solar"), "solar", substr(lcm_from, 1, 3))))
  ][lcm_from != lcm_to
    , keyby = .(scenario, lusp_scenario, year, lcm_from, lcm_to, country)
    , .(area = sum(area))] %>% 
    as_tibble() %>% 
  ...
  luc_areas <- luc_areas %>% 
    ...
    left_join(prop_bioenergycrop, by = c("lusp_scenario" = "scenario")) %>% 
  ...
  gwp_luc <- bind_rows(
    ...
    luc_areas %>%
      ...
      group_by(scenario, lusp_scenario, year, country, category_1) %>%
    ...
    luc_renewable_areas %>% 
      inner_join(ghg_pars_luc$gwp_luc_renewables_soil, by = c("lcm_from", "lcm_to", "country")) %>%
      mutate(year = year.x + year.y) %>%
      select(-year.x, -year.y) %>%
      filter(year <= 2100) %>% 
      group_by(scenario, lusp_scenario, year, country, category_1) %>% 
  ...
  livestock_data <- food_results_basic %>%
    mutate(d = purrr::map(food, ~.[[4]])) %>% 
    select(-food, -i) %>% 
    unnest(cols = c(data, d)) %>% 
    # Filter for high/low - low syntax: doesn't contain high - to include LUSP scenarios too
    filter(case_when(energy_ambition == "high" ~ str_detect(scenario, "high"),
                     energy_ambition == "low" ~ !str_detect(scenario, "high"))) %>% 
    # Add lusp scenario class
    mutate(lusp_scenario = str_sub(scenario, 1, 1)) %>% 
    # Combine Eggs & Poultry
    mutate(animal = ifelse(animal == "Eggs", "Poultry", animal)) %>%
    group_by(scenario, lusp_scenario, year, country, animal) %>%
    summarise_all(sum) %>% 
    ungroup() %>% 
    # Annualise (linear interpolate between years)
    full_join(years, by = "year") %>%
    filter(year_ <= 2050) %>% 
    group_by(scenario, lusp_scenario, country, animal) %>%
    mutate_at(vars(me, ge, cp), ~zoo::na.approx(ifelse(year == year_, ., NA))) %>% 
    ungroup() %>% 
    select(-year) %>% 
    rename(year = year_) %>% 
    # Add rows for deer, goats and horses
    bind_rows((.) %>% 
                select(scenario, lusp_scenario, year, country) %>% 
                unique() %>% 
                crossing(animal = c("Deer", "Goats", "Horses"))) 
  
  
  # Pull out ha per crop
  crop_ha <- food_results_basic %>%
    ungroup() %>% 
    ...
    full_join(years, by = "year") %>%
    filter(year_ <= 2050) %>% 
    group_by(scenario, lusp_scenario, country, crop_fine, organic) %>%
    mutate_at(vars(area, tonnage), ~zoo::na.approx(ifelse(year == year_, ., NA), na.rm = FALSE)) %>% 
    ungroup() %>% 
    select(-year) %>% 
    rename(year = year_) %>% 
    drop_na(crop_fine)
  ...
  crop_tonnage <- food_results_basic %>%
    ungroup() %>% 
    ...
    group_by(scenario, lusp_scenario, year, country) %>% 
    summarise(tonnage = sum(tonnage), .groups = "drop") %>% 
    # Annualise (linear interpolate between years)
    full_join(years, by = "year") %>%
    filter(year_ <= 2050) %>% 
    group_by(scenario, lusp_scenario, country) %>%
    mutate_at(vars(tonnage), ~zoo::na.approx(ifelse(year == year_, ., NA), na.rm = TRUE)) %>% 
    ungroup() %>% 
    select(-year) %>% 
    rename(year = year_)
  
  # Pull out feed imports
  feed_imports <- food_results_basic %>%
    ungroup() %>% 
    ...
    filter(year_ <= 2050) %>% 
    group_by(scenario, lusp_scenario, country) %>%
    mutate_at(vars(mj_imported), ~zoo::na.approx(ifelse(year == year_, ., NA), na.rm = TRUE)) %>% 
    ungroup() %>% 
    select(-year) %>% 
    rename(year = year_)
  ...
  spring_crop_props <- crop_ha %>%
    mutate(type = case_when(crop_fine %in% c("Permanent grass", "Temporary grass") ~ crop_fine,
                            ...
                            TRUE ~ "Other crop")) %>% 
    group_by(scenario, lusp_scenario, year, country, type) %>% 
  ...
  gwp_synthn_agsoils_n2o <- crop_ha %>% 
    ...
    left_join(uptake_lowcarbonfarming %>% filter(measure == "nue_crop") %>% select(-measure), by = c("lusp_scenario" = "scenario", "year")) %>%
    ...
    left_join(uptake_lowcarbonfarming %>% filter(measure == "nue_grass") %>% select(-measure), by = c("lusp_scenario" = "scenario", "year")) %>%
    mutate(yield_growth = 1,
           rate = case_when(crop_fine %in% c("Permanent grass", "Temporary grass", "Temporary grass (new ley)") & yield_growth == 1 ~ rate * (1 - uptake), 
                            # crop_fine %in% c("Permanent grass", "Temporary grass", "Temporary grass (new ley)") & yield_growth > 1 ~ rate + rate * (yield_growth - 1) * (1 - uptake),
                            crop_fine %in% c("Permanent grass", "Temporary grass", "Temporary grass (new ley)") & yield_growth > 1 ~ rate,
    ...
    left_join(uptake_lowcarbonfarming %>% filter(measure == "grass_legume_mix") %>% select(-measure), by = c("lusp_scenario" = "scenario", "year")) %>%
    mutate(yield_growth = 1,
    ...
    left_join(uptake_lowcarbonfarming %>% filter(measure == "cover_crops") %>% select(-measure), by = c("lusp_scenario" = "scenario", "year")) %>%
    ...
    left_join(uptake_lowcarbonfarming %>% filter(measure == "loosen_soils") %>% select(-measure), by = c("lusp_scenario" = "scenario", "year")) %>%
    ...
    group_by(scenario, lusp_scenario, year, country, ag_sector, ghg, category_2) %>%
  ...
  gwp_otherag <- crop_tonnage %>% 
    ...
    left_join(uptake_lowcarbonfarming %>% filter(measure == "electrification") %>% select(-measure), by = c("lusp_scenario" = "scenario", "year")) %>% 
  ...
  gwp_c_agsoils <- crop_ha %>% 
    ...
    left_join(uptake_lowcarbonfarming %>% filter(measure == "grass_leys") %>% select(-measure), by = c("lusp_scenario" = "scenario", "year")) %>% 
  ...
  gwp_entericferm_ch4 <- livestock_data %>%
    ...
    left_join(uptake_lowcarbonfarming %>% filter(measure == "additive") %>% select(-measure), by = c("lusp_scenario" = "scenario", "year")) %>%
    ...
    left_join(uptake_lowcarbonfarming %>% filter(measure == "breeding_genomics") %>% select(-measure), by = c("lusp_scenario" = "scenario", "year")) %>%
  ...
  gwp_manureman_ch4 <- livestock_data %>% 
    ...
    left_join(uptake_lowcarbonfarming %>% filter(measure == "precision_feeding") %>% select(-measure), by = c("lusp_scenario" = "scenario", "year")) %>%
    mutate(gwp_t = ifelse(animal %in% c("Milk", "Pig"), 
                          gwp_t * (1 - (uptake * 0.02)),
                          gwp_t)) %>% 
    select(-uptake) %>% 
    # Cover slurry tanks: Reduce CH4 emissions by 47% for liquid manure
    left_join(uptake_lowcarbonfarming %>% filter(measure == "cover_slurry") %>% select(-measure), by = c("lusp_scenario" = "scenario", "year")) %>%
  ...
  gwp_manureman_directn2o <- livestock_data %>%
    ...
    left_join(uptake_lowcarbonfarming %>% filter(measure == "hsg_dairy") %>% select(-measure), by = c("lusp_scenario" = "scenario", "year")) %>%
  ...
  gwp_manure_agsoils_n2o <- livestock_data %>% 
    ...
    left_join(uptake_lowcarbonfarming %>% filter(measure == "cover_crops") %>% select(-measure), by = c("lusp_scenario" = "scenario", "year")) %>%
    left_join(spring_crop_props, by = c("scenario", "lusp_scenario", "year", "country")) %>% 
  ...
  gwp_sewage_agsoils_n2o <- pop_change %>% 
    ...
    select(scenario, lusp_scenario, year, country, ag_sector, ghg, gwp_t, category_2)
  ...
  gwp_ag <- bind_rows(gwp_synthn_agsoils_n2o,
                      ...
                      gwp_feed_imported) %>% 
    group_by(scenario, lusp_scenario, year, country, category_2, ag_sector, ghg) %>%
    summarise(gwp_t = sum(gwp_t), .groups = "drop") %>% 
  ...
  peat_areas <- scenarios_areas[peat == 1 & lcm != "f.w"
                                , keyby = .(scenario, lusp_scenario, year, country, lcm)
                                , .(area = sum(area))] %>% 
    as_tibble() %>% 
    # Add lusp scenario class
    mutate(lusp_scenario = str_sub(scenario, 1, 1)) %>% 
    # Complete missing years (fill area with 0)
    complete(year, nesting(scenario, lusp_scenario, country, lcm), fill = list(area = 0)) %>%
    # Annualise (interpolate between 5 years)
    left_join(years, by = "year") %>%
    filter(year_ <= 2050) %>% 
    group_by(scenario, lusp_scenario, country, lcm) %>% 
  ...
  peat_areas <- peat_areas %>%
    ...
    left_join(peat_areas %>%
              ...
              by = c("scenario", "lusp_scenario", "country", "lcm", "year")) %>%
    mutate(area_new = area - area_existing) %>%
    select(-area) %>%
    # Tidy
    gather(condition, area, -scenario, -lusp_scenario, -country, -lcm, -year) %>%
    ...
    bind_rows(peat_areas %>%
                ...
                group_by(scenario, lusp_scenario, country, lcm, year, condition) %>% 
                summarise(area = sum(area), .groups = "drop")) %>%
    select(scenario, lusp_scenario, year, country, lcm, condition, area) 
  ...
  gwp_peat <- peat_areas %>%
    ...
    group_by(scenario, lusp_scenario, country, ghg, year, lcm) %>%
    ...
    group_by(scenario, lusp_scenario, year, country, ghg, lcm) %>% 
  ...
  gwp_peat <- gwp_peat %>% 
    inner_join(peat_areas, by = c("scenario", "lusp_scenario", "year", "country", "condition", "lcm")) %>%
  ...
  gwp_peat <- gwp_peat %>% 
    ...
    group_by(scenario, lusp_scenario, country, year, category_1, category_2, ghg) %>% 
    summarise(gwp_t = sum(gwp_t * area), .groups = "drop")
  ...
  intertidal_new_ha <- scenarios_change_areas[lcm_to == "s.m" & lcm_from != "s.m"
                                              , keyby = .(scenario, lusp_scenario, year, country)
  ...
  gwp_intertidal <- bind_rows(ghg_pars_intertidal$intertidal_existing_ha %>%
                                ...
                                group_by(year, scenario, lusp_scenario, country, category, ghg) %>% 
                              ...
                              intertidal_new_ha %>% 
                                ...
                                select(year, country, category, ghg, gwp_t, scenario, lusp_scenario)) %>%
    ...
    select(scenario, lusp_scenario, country, year, category_1, category_2, ghg, gwp_t) 
  

  ## BIOENERGY INPUTS ---------------------------------------------
  print("Doing bioenergy...")
  # Calculate bioenergy crop areas (on arable)
  bioenergy_areas <- scenarios_change_areas[, ':='(area = ifelse(lcm_to == "wind_a.h_energy", area*(1-wind_footprint), area),
                                                   lcm_to = ifelse(lcm_to == "wind_a.h_energy", substring(lcm_to, first = 6), lcm_to),
                                                   lcm_from = ifelse(lcm_from == "wind_a.h_energy", substring(lcm_from, first = 6), lcm_from))
  ][lcm_to == "a.h_energy" & lcm_from != "a.h_energy"
    , keyby = .(scenario, lusp_scenario, year, country, lcm_to)
    , .(area = sum(area))] %>%
    as_tibble()
  ...
  if(nrow(bioenergy_areas) > 0){
    ...
    gwp_bioenergy <- bioenergy_areas %>%
      ...
      group_by(scenario, lusp_scenario, year, country, ghg, category_1, category_2) %>%
      summarise(gwp_t = sum(gwp_t * area), .groups = "drop")
  } else{
    gwp_bioenergy <- NULL
  }
  ...
  extant_wood_areas <- scenarios_areas[year == 2015 & lcm %in% c("b.w", "c.w", "c.w_pinewood")
                                       , keyby = .(lusp_scenario, scenario, country, lcm, yc_sbi, yc_sok, yc_sp, yc_ss)
                                       , .(area = sum(area))] %>% 
    as_tibble() %>% 
    # Cross with all years to 2050
    crossing(year = 2015:2050) %>% 
    # Join with area converted to other land-use
    left_join(scenarios_change_areas[, ':='(area = ifelse(lcm_to == "wind_c.w", area*wind_footprint, area))
    ][lcm_from %in% c("b.w", "c.w", "c.w_pinewood") & lcm_from != lcm_to
                                     , keyby = .(lusp_scenario, scenario, year, country, lcm_from, yc_sbi, yc_sok, yc_sp, yc_ss)
                                     , .(area_lost = sum(area))
    ][, ':='(lcm = lcm_from,
             lcm_from = NULL)] %>% 
      as_tibble() %>% 
      ...
      rename(year = year_) %>%
      arrange(scenario, lusp_scenario, year, country),
    by = c("lusp_scenario", "scenario", "country", "year", "lcm", "yc_sbi", "yc_sok", "yc_sp", "yc_ss")) %>% 
    ...
    group_by(scenario, lusp_scenario, year, country, lcm, type, yc) %>% 
    summarise(area = sum(area), .groups = "drop") %>% 
    # Cross with age cohort distribution 
    # left_join(ghg_pars_wood$existing_wood_age, by = c("type", "lcm")) %>%
    # left_join(ghg_pars_wood$existing_wood_age, by = c("country", "lcm")) %>%
  ...
  new_wood_areas <- scenarios_change_areas[, ':='(area = ifelse(lcm_to == "wind_i.g_energy", area*(1-wind_footprint), area),
                                                  lcm_to = ifelse(lcm_to == "wind_i.g_energy", substring(lcm_from, first = 6), lcm_to),
                                                  lcm_from = ifelse(lcm_from == "wind_i.g_energy", substring(lcm_from, first = 6), lcm_from))
  ][lcm_to %in% c("b.w", "c.w", "c.w_pinewood", "i.g_energy", "test") & lcm_from != lcm_to
                                           , keyby = .(lusp_scenario, scenario, year, country, lcm_to, yc_sbi, yc_sok, yc_sp, yc_ss, natregen)
                                           , .(area = sum(area))
  ][, ':='(lcm = lcm_to,
           lcm_to = NULL)] %>% 
    as_tibble() %>% 
    ...
    mutate(natregen = ifelse(type %in% c("Semi-natural broadleaved woodland (unmanaged)", "Semi-natural coniferous woodland (unmanaged)") & natregen == 1,
           ...
           yc = case_when(type != "Managed coniferous woodland" & yc <= 4 ~ 4,
                          ...
                          TRUE ~ yc)) %>% 
    # Sum area per woodland type/YC
    group_by(scenario, lusp_scenario, year, country, lcm, type, natregen, yc) %>% 
    summarise(area = sum(area), .groups = "drop") %>% 
    # Cross with age cohort distribution 
  ...
  extinct_wood_areas <- scenarios_change_areas[, ':='(area = ifelse(lcm_to == "wind_c.w", area*wind_footprint, area))
  ][lcm_from %in% c("b.w", "c.w") & lcm_from != lcm_to
    , keyby = .(lusp_scenario, scenario, year, country, lcm_from, yc_sbi, yc_sok, yc_sp, yc_ss)
    , .(area = sum(area))
  ][, ':='(lcm = lcm_from,
           lcm_from = NULL)] %>% 
  ...
  # First calculate flux from extinct woodland (special case due to early harvest - need to recalc HWP and debris fluxes)
  gwp_woodland <- bind_rows(extant_wood_areas %>%
    ...
    # Summarise
    group_by(scenario, lusp_scenario, country, year, category_1, category_2) %>%
  ...
  agroforestry_areas <- scenarios_change_areas[, ':='(area = ifelse(lcm_to %chin% c("wind_a.h_silvoa", "wind_i.g_silvop", "wind_a.h_organic_silvoa", "wind_i.g_organic_silvop"), area*(1-wind_footprint), area),
                                                      lcm_to = ifelse(lcm_to %chin% c("wind_a.h_silvoa", "wind_i.g_silvop", "wind_a.h_organic_silvoa", "wind_i.g_organic_silvop"), substring(lcm_to, first = 6), lcm_to))
  ][lcm_to %chin% c("a.h_silvoa", "i.g_silvop", "a.h_organic_silvoa", "i.g_organic_silvop") & lcm_from %chin% c("a.h", "i.g", "a.h_organic", "i.g_organic")
    , keyby = .(lusp_scenario, country, scenario, year, lcm_to)
    , .(area = sum(area))
  ][, lcm := lcm_to
  ][, lcm_to := NULL] %>%
    as_tibble() %>% 
  ...
  gwp_agroforestry <- bind_rows(
    ...
    agroforestry_areas %>% 
      ...
      group_by(scenario, lusp_scenario, country, year, type) %>%
    ...
    agroforestry_areas %>% 
      ...
      # Summarise and tidy
      group_by(lusp_scenario, scenario, country, year, ghg, type) %>% 
  ...
  ][, keyby = .(lusp_scenario, country, scenario, year, lcm_to, yc)
    , .(area = sum(area))
  ][, lcm := lcm_to
  ][, lcm_to := NULL] %>%
    as_tibble() %>% 
  ...
  gwp_woodpasture <- woodpasture_areas %>% 
    ...
    # Sum annual flux across woodland types
    group_by(scenario, lusp_scenario, country, year) %>% 
  ...
  gwp_hedges <- bind_rows(
    ...
    hedge_add %>%
      ...
      filter(year <= 2100) %>%
      # Summarise and tidy
      group_by(scenario, country, year, ghg) %>%
    ...
    # Add eLUSP scenario labels
    rename(lusp_scenario = scenario) %>% 
    left_join(scenario_names,
              by = "lusp_scenario") %>% 
    select(scenario, lusp_scenario:category_2) 
  ...
  hwp_combined <- bioenergy_areas %>% 
    ...
    group_by(scenario, lusp_scenario, year, country, hwp_type) %>% 
  ...
  hwp_combined <- agroforestry_areas %>% 
    ...
    group_by(scenario, lusp_scenario, year, country, hwp_type) %>% 
    summarise(hwp = sum(area * area_multiplier * clearfell_stock) * 12/44 * 2,
              .groups = "drop") %>% 
    bind_rows(hwp_combined)
...
save(do_ghg_fun, 
     ghg_pars_luc,
     ghg_pars_ag, pop, manure_systems, lowcarbonfarming,
     ghg_pars_peat,
     ghg_pars_intertidal,
     ghg_pars_bioenergy,
     ghg_pars_wood, 
